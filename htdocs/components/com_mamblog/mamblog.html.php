...
class HTML_mamblog {
	...
	function showCommentLink( $id, $count, $link ) {
		...
		echo $link;
		echo _mbtextp('VIEW_ADD_COMMENTS')."</a> ($count)";
		echo "<br /><br />";
	}
...
<?php
	...
	function editBlog( $id, $blog, $header, $head, $frm, $headattribs, $frmattribs ) {
		...
		$lang['_TITLE']               = _mbtextp('ENTRY_TITLE');
		$lang['_CONTENT']             = _mbtextp('ENTRY_CONTENT');
		$lang['_BUTTON_MAMBLOG']      = _mbtextp('ENTRY_SAVE_BUTTON');
		$lang['_EXTRAS']              = _mbtextp('ENTRY_APPEARANCE');
		$lang['_ATTRIBS']             = _mbtextp('ENTRY_ATTRIBUTES');
		$lang['_ALLOWCOMMENTS']       = _mbtextp('ENTRY_ALLOW_COMMENTS');
		$lang['_SHOWCOMMENTS']        = _mbtextp('ENTRY_SHOW_COMMENTS');
		$lang['_FRONTPAGE']           = _mbtextp('ENTRY_FRONTPAGE');
		$lang['_ARCHIVED']            = _mbtextp('ENTRY_ARCHIVED');
		$lang['_PUBLISHED']           = _mbtextp('ENTRY_PUBLISHED');
		$lang['_ERROR']               = _mbtextp('ERROR');
		$lang['_BUTTON_DELETE']       = _mbtextp('ENTRY_DELETE_BUTTON');
		$lang['_BLOG_CONFIRM_DELETE'] = _mbtextp('ENTRY_CONFIRM_DELETE');
...
CONTENT;
	...
	function show( $blog, $mask=0, $gid, $option ) {
		...
		switch (intval($blog->state)) {
			case 0: $state  = _mbtextp('ENTRY_STAT_UNPUBLISHED'); break;
			case 1: $state  = _mbtextp('ENTRY_STAT_PUBLISHED'); break;
			case -1: $state = _mbtextp('ENTRY_STAT_ARCHIVED'); break;
			default: $state = _mbtextp('ENTRY_STAT_UNKNOWN'); break;
		}
...
<?php	if (!($mask&MASK_HIDECREATEDATE)) { ?>
		<tr>
			<td class="createdate" colspan="4" align="left"> (<?php echo $create_date; ?>) <?php echo _mbtextp('WRITTEN_BY'); ?> <a href="<?php echo sefRelToAbs( "index.php?option=$option&amp;Itemid=$Itemid&amp;task=show&amp;action=user&amp;id=$blog->created_by" ); ?>"><?php echo $blog->author; ?></a></td>
		</tr>
<?php	} else { ?>
		<tr>
			<td class="createdate" colspan="4" align="left"> <?php echo _mbtextp('WRITTEN_BY'); ?> <a href="<?php echo sefRelToAbs( "index.php?option=$option&amp;Itemid=$Itemid&amp;task=show&amp;action=user&amp;id=$blog->created_by" ); ?>"><?php echo $blog->author; ?></a></td>
		</tr>
<?php	} ?>
<?php	if ( ( $my->id == $blog->created_by ) ) { ?>
		<tr>
			<td class="createdate" colspan="4" align="left"><?php echo _mbtextp('STATE')." ".$state; ?></td>
		</tr>
<?php	} ?>
		<tr>
...
<?php		if($blog->access <= $gid ) { ?>
				<a href="<?php echo sefRelToAbs( "index.php?option=$option&amp;Itemid=$Itemid&amp;task=show&amp;action=view&amp;id=$blog->id&amp;Itemid=$Itemid" ); ?>" class="readon"><?php echo _mbtext('READ_ON');?></a>
<?php		} else { ?>
				<a href="<?php echo sefRelToAbs( "index.php?option=com_registration&amp;task=register" ); ?>" class="readon"><?php echo _mbtextp('READ_ON_REGISTER');?></a>
<?php		} ?>
			</td>
		</tr>
<?php	} ?>
  ...
  <tr> 
    <td colspan="3" align="center"><a href="javascript:window.history.go(-1);"><?php echo _mbtextp('BACK'); ?></a></td>
  </tr>
  <?php	} ?>
	</table>
...
<?php
	...
	function showLinks( &$rows, $limitstart, $limit ) {
		global $Itemid, $option, $cfg_mamblog;
...
<table cellpadding="0" cellspacing="5" border="0" width="100%" class="contentpaneopen">
  <tr> 
    <td> <strong><?php echo _mbtextp('MORE'); ?></strong> </td>
  </tr>
