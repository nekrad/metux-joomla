...
$id = intval($id);
if ( $my->username ) {
	if ( mosMenuCheck( "", "$option", "edit", $cfg_mamblog{'current_user'}->gid ) ) {
		switch( $action ) {
			case "modify":
...
function deleteMamblog( $id, $uid, $option ) {
	global $cfg_mamblog, $database, $my;
	...
	if ( $id ) {
		...
		$msg = _mbtextp('NO_BLOG_ENTRY');
	}

	if ( $msg ) {
		showError( $msg );
	} else {
		HTML_mamblog::blogMessage(_mbtextp('ENTRY_DELETED_LONG'), _mbtexp('ENTRY_DELETED') );
	}
}

function editMamblog( $id, $uid, $option, $err="", $mamblog="" ) {
	global $cfg_mamblog, $database;
	
	$header = _mbtextp('NEW_ENTRY');

	// Check if we need to read an existing Mamblog
	if ( $mamblog ) {
		$header = _mbtext('EDIT_BLOG_ENTRY');
	} else {
		$mamblog = new mosContent( $database );
		$mamblog->load( $id );
	
		if ( $mamblog->id && $mamblog->created_by == $uid ) {
			$header = _mbtextp('EDIT_BLOG_ENTRY');
		} else if ( $mamblog->id ) {
			$err .= "<ul><li>"._mbtextp('ERROR_INVALID_ENTRY')."</li></ul>";
			$mamblog = new mosContent( $database );
			$id = 0;
		} else {
			$id = 0;
	...
	{
		echo _mbtextp('CHECKED_OUT_BY').
		     $mamblog->editor.
		     _mbtextp('CHECKED_OUT_AT').
		     $mamblog->checked_out_time;
	} 
	else 
	{
		...
		if ( $cfg_mamblog['useattribs'] ) {
			...
			if ( $cfg_mamblog['use_width'] ) {
				$head[] = _mbtextp('WIDTH') . " (" . $cfg_mamblog['min_width'] . " - " . $cfg_mamblog['max_width'] . ")";
				$frm[] = '<input class="inputbox" type="text" name="width" size="4" maxlength="4" value="' . @$attribs->width . '" />';
			}
			
			if ( $cfg_mamblog['use_height'] ) {
				$head[] = _mbtextp('HEIGHT') . " (" . $cfg_mamblog['min_height'] . " - " . $cfg_mamblog['max_height'] . ")";
		...
		$yesno = array();
		$yesno[] = mosHTML::makeOption( '0', _mbtextp('NO') );
		$yesno[] = mosHTML::makeOption( '1', _mbtextp('YES') );

		$frmattribs = array();
		$headattribs = array();

		if ( $cfg_mamblog['use_allowcomments'] ) {
			$headattribs[] = _mbtextp('ENTRY_ALLOW_COMMENTS');
		...
		if ( $cfg_mamblog['use_showcomments'] ) {
			$headattribs[] = _mbtextp('ENTRY_SHOW_COMMENTS');
		...
		if ( $cfg_mamblog['use_frontpage'] ) {
			$headattribs[] = _mbtextp('ENTRY_FRONTPAGE');
			$frmattribs[] = mosHTML::selectList( $yesno, 'mask', 'class="inputbox" size="1"', 'value', 'text', $mamblog->mask );
		}

		if ( $cfg_mamblog['use_state'] ) {
			$headattribs[] = _mbtextp('ENTRY_PUBLISHED_HEADER');
			$state[] = mosHTML::makeOption( '0',  _mbtextp('ENTRY_STAT_UNPUBLISHED' ) );
			$state[] = mosHTML::makeOption( '1',  _mbtextp('ENTRY_STAT_PUBLISHED') );
			$state[] = mosHTML::makeOption( '-1', _mbtextp('ENTRY_STAT_ARCHIVED' ) );
			$frmattribs[] = mosHTML::selectList( $state, 'state', 'class="inputbox" size="1"', 'value', 'text', $mamblog->state );
		}

		if ( $cfg_mamblog['use_access'] ) {
			$headattribs[] = _mbtextp('ENTRYA_ACCESS');
			$access[] = mosHTML::makeOption( '0', _mbtextp('ENTRY_ACCESS_ALL') );
			$access[] = mosHTML::makeOption( '1', _mbtextp('ENTRY_ACCESS_REGISTERED') );
			$access[] = mosHTML::makeOption( '2', _mbtextp('ENTRY_ACCESS_USER') );
...
function saveMamblog( $id, $uid, $option, $arrmamblog=array() ) {
	...
	$mamblog->sectionid = $cfg_mamblog{'sectionid'};
		...
		if ( $cfg_mamblog['use_width'] && $attribs->width ) {
			if ( substr($attribs->width, -1, 1) == "%" ) {
				if ( intval( substr( $attribs->width, 0, -1 ) ) < 1 ||
					...
					$err .= "<li>"._mbtextp('ERROR_INVALID_WIDTH')."</li>\n";
				}
			} else {
		...
		if ( $cfg_mamblog['use_height'] && $attribs->height ) {
			if ( substr($attribs->height, -1, 1) == "%" ) {
				if ( intval( substr( $attribs->height, 0, -1 ) ) < 1 ||
					...
					$err .= "<li>"._mbtextp('ERROR_INVALID_HEIGHT')."</li>\n";
				}
			} else {
	...
	if ( $cfg_mamblog['use_allowcomments'] ) {
		if ($attribs->allowcomments == "") {
			...
			$err .= "<li>"._mbtextp('ERROR_INVALID_ALLOW_COMMENTS')."</li>\n";
		}
	}
	if ( $cfg_mamblog['use_showcomments'] ) {
		if ($attribs->showcomments == "") {
			...
			$err .= "<li>"._mbtextp('ERROR_INVALID_SHOW_COMMENTS')."</li>\n";
		}
	}
	...
	if ( $cfg_mamblog['use_state'] ) {
		if ($mamblog->state == "") {
			...
			$err .= "<li>"._mbtextp('ERROR_INVALID_PUBLISHED')."</li>\n";
		}
	} else {
	...
	if ( $cfg_mamblog['use_frontpage'] ) {
		if ( $mamblog->mask != 0 && $mamblog->mask != 1 ) {
			$err .= "<li>"._mbtextp('ERROR_INVALID_FRONTPAGE')."</li>\n";
		}
	} else {
	...
	if ( $cfg_mamblog['use_access'] ) {
		if ($mamblog->access == "") {
			...
			$err .= "<li>"._mbtextp('ERROR_INVALID_ACCESS')."</li>\n";
		}
	} else {
	...
	if ($err) {
		$err = "<ul>" . $err . "</ul>"._mbtextp('ERROR_HELP');
...
function saveToDB( $id, $uid, $option, $mamblog ) {
	global $database, $mainframe, $cfg_mamblog;
	...
	if ( !$mamblog->catid ) {
		$mamblog->catid = $cfg_mamblog{'catid'};
	}
...
#	$fp->updateOrder();
	...
	HTML_mamblog::blogMessage(_mbtextp('ENTRY_SAVED'),_mbtextp('ENTRY_SAVE'));
}
