...
$gid = $cfg_mamblog{'current_user'}->gid;
switch( $action ) {
	case "user":
...
function showBlog( $id, $uid, $option, $gid, $mask=0, $notext=0 ) {
	global $cfg_mamblog, $database, $mainframe;
	...
	$gid = 10000;	// ugly hack, no idea WTF this gid crap is about ;-o
	...
	if ( $row->id ) {
		...
		showError(_mbtextp('ERROR_INVALID_ENTRY'));
	}
}
...
#	$access = !$mainframe->getCfg( 'shownoauth' );	// requires honouring of access
	...
	if ( $type == "user" || $type == "userarchive" ) {
		...
		if ( !$database->loadObject( $userobj ) ) {
			showError(_mbtextp('ERROR_INVALID_USER').": ".$id );
			return false;
		}
		$id = $userobj->id;
	}
	...
	if ( $type == "date") {
		if ( is_date( "$id 00:00:00" ) ) {
			...
			showError(_mbtextp('ERROR_INVALID_DATE')." $id" );
			return false;
		}
	}
	#if ( $type == "frontpage" ) {
	...
	$where[] = "a.sectionid='{$cfg_mamblog{'sectionid'}}'";
	...
	switch ( $type ) {
		case "user":
			$header = _mbtextp('SHOW_BY_USER')." ".$userobj->author; break;
		case "userarchive":
			$header = _mbtextp('SHOW_USER_ARCHIVE')." ".$userobj->author; break;
		case "date":
			$header = _mbtextp('SHOW_BY_DATE')." ".mosFormatDate($id); break;
		default:
			#$header = _mbtextp('SHOW_ALL');
	...
	if ( $n > 0 ) {
		...
		if ( $count < $blogentries ) {
			echo "<br clear='all' /><a href='".sefRelToAbs("index.php?option=$option&amp;Itemid=$Itemid&amp;task=show&amp;action=$type&amp;id=$id&amp;ignorecount=1")."'><strong>"._mbtextp('SHOW_ALL_ENTRIES')."</strong></a><br />";
			
		}
	} else {
		echo _mbtextp('NO_ENTRIES');
	}

	if ( $cfg_mamblog['showarchivelink'] && $type == "user" && intval( $id ) ) {
		echo "<br clear='all' /><a href='".sefRelToAbs( "index.php?option=$option&amp;Itemid=$Itemid&amp;task=show&amp;action=userarchive&amp;id=$id" ) . "'><strong>"._mbtextp('USER_ARCHIVE')."</strong></a><br />";
	}

	/* if we're showing the current user's own bloglist, display an add button */
	if ($uid == $cfg_mamblog{'current_user'}->id)
	{
	    print '<div class="mamblog_add_blog_entry"><a href="'.
		$cfg_mamblog{'url_render'}->addBlogEntry().'">'.
		_mbtextp('ADD_BLOG_ENTRY').'</a></div>';
	}
}
...
function checkLength( $text ) {
	...
	if ( $maxchars > 0 && $maxchars < $strlen ) {
		...
		$text .= " (".( $strlen - $maxchars )." "._mbtextp('MORE_CHARS').")";
	} else {
		$text = safeStrip( $text );
	}
