...
class BlogController extends JController
{
	...
	function delete_mypost()
	{
		...
		$db =& JFactory::getDBO();
		$query = "SELECT id, user_id, post_image FROM #__blog_postings WHERE id = $id";
		$db->setQuery( $query );
		...
		if($rows[0]->id){
			...
			$query = "DELETE FROM #__blog_postings WHERE id = ".$rows[0]->id;
			$db->setQuery( $query );
			if (!$db->query())
			{
 				$link = JRoute::_('index.php?option=com_blog&view=myposts&Itemid='.$Itemid, false);
				...
				$query = "DELETE FROM #__blog_comment WHERE post_id = ".$rows[0]->id;
				$db->setQuery( $query );
				$db->query();
			}
