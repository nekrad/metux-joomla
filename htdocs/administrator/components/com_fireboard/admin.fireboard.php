...
define('JPATH_LEGACY', 		JPATH_COMPONENT.'/../../../plugins/system/legacy/');
define('FIREBOARD_FRONTEND', 	JPATH_COMPONENT.'/../../../components/com_fireboard/');
define('FIREBOARD_BACKEND',	JPATH_COMPONENT);

include_once (FIREBOARD_BACKEND.'/fireboard_config.php');
require_once (JPATH_LEGACY.'/functions.php');
require_once (JPATH_LEGACY.'/dbtable.php');
require_once (JPATH_LEGACY.'/menubar.php');
require_once (JPATH_LEGACY.'/html.php');

global $database;
$database = &JFactory::getDBO();

require_once (FIREBOARD_FRONTEND.'/class.fireboard.php');
require_once ($mainframe->getPath('admin_html'));

//Get right Language file
if (file_exists(FIREBOARD_BACKEND.'/language/'.$mainframe->getCfg('lang').'.php'))
    include (FIREBOARD_BACKEND.'/language/'.$mainframe->getCfg('lang').'.php');
else
    include (FIREBOARD_BACKEND.'/language/english.php');
...
function showConfig($option)
{
    ...
    $database = &JFactory::getDBO();
    $configfile = FIREBOARD_BACKEND.'/fireboard_config.php';
    ...
    if ($dir = @opendir(FIREBOARD_FRONTEND.'/template'))
    {
        while (($file = readdir($dir)) !== false)
        {
            if ($file != ".." && $file != ".")
            {
                if (is_dir(FIREBOARD_FRONTEND.'/template/' . $file))
                {
                    if (!($file[0] == '.')) {
                        $filelist[] = $file;
                    }
...
function pruneforum($database, $option)
{
    ...
    // NEKRAD: a little fix for warning on empty list
    if (!is_array($forums_list))
	$forums_list = array();
...
function smileypath()
{
    global $mainframe, $mosConfig_lang, $fbConfig;

    if (is_dir(FIREBOARD_FRONTEND.'/template/'.$fbConfig['template'].'/images/'.$mosConfig_lang.'/emoticons')) {
        ...
        $smiley_abs_path = FIREBOARD_FRONTEND.'/template/'.$fbConfig['template'].'/images/'.$mosConfig_lang.'/emoticons';
    }
    else {
        $smiley_live_path = $mainframe->getCfg('live_site') . '/components/com_fireboard/template/default/images/'.$mosConfig_lang.'/emoticons';
        $smiley_abs_path = FIREBOARD_FRONTEND.'/template/default/images/'.$mosConfig_lang.'/emoticons';
    }
...
{
	global $mainframe, $mosConfig_lang, $order;
	$limit = intval(mosGetParam($_POST, 'limit', 10));
	$limitstart = intval(mosGetParam($_POST, 'limitstart', 0));
	$database = &JFactory::getDBO();
...
function rankpath()
{
    ...
    if (is_dir(FIREBOARD_FRONTEND.'/template/'.$fbConfig['template'].'/images/'.$mosConfig_lang.'/ranks')) {
        ...
        $rank_abs_path = FIREBOARD_FRONTEND.'/template/'.$fbConfig['template'].'/images/'.$mosConfig_lang.'/ranks';
    }
    else {
        $rank_live_path = $mainframe->getCfg('live_site') . '/components/com_fireboard/template/default/images/'.$mosConfig_lang.'/ranks';
        $rank_abs_path = FIREBOARD_FRONTEND.'/template/default/images/'.$mosConfig_lang.'/ranks';
    }
...
function com_install_fireboard($mode = 1)
{
    ...
    add column `class_sfx` varchar(20) NOT NULL  after `headerdesc`, COMMENT='';");
        ...
        dircopy(JPATH_SITE."/components/com_joomlaboard/uploaded", JPATH_SITE."/images/fbfiles", false);
        dircopy(JPATH_SITE."/components/com_joomlaboard/avatars", JPATH_SITE."/images/fbfiles/avatars", false);

        $database->setQuery("update #__fb_attachments set filelocation = replace(filelocation,'com_joomlaboard','com_fireboard');");
        $database->query();

        $database->setQuery("update #__fb_attachments set filelocation = replace(filelocation,'".JPATH_SITE."/components/com_fireboard/uploaded','/images/fbfiles');");
        if ($database->query()) {
