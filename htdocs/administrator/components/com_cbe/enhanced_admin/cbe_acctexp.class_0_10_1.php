...
//if(in_array($mosConfig_dbprefix."cbe_members", $tables)) { // CB Integration
//	/**
...
//	include_once($_CB_adminpath."/imgToolbox.class.php");
//
//	$UElanguagePath=$mainframe->getCfg( 'absolute_path' ).'/components/com_cbe/enhanced/language';
//	if (file_exists($UElanguagePath.'/'.$mosConfig_lang.'.php')) {
//	  include_once($UElanguagePath.'/'.$mosConfig_lang.'.php');
//	} else include_once($UElanguagePath.'/english.php');
//
//	$form=mosGetParam( $_REQUEST, 'reportform', 1 );
//	$uid=mosGetParam( $_REQUEST, 'uid', 0 );
//	$act=mosGetParam( $_REQUEST, 'act', 1 );
...
//		mosRedirect( $mosConfig_live_site . "/index.php?option=$option&task=lostPassword&mosmsg="._ERROR_PASS );
//	}
//
...
//	}
//
//	mosRedirect( $mosConfig_live_site . "/index.php?Itemid=$Itemid&mosmsg="._NEWPASS_SENT );
//}
//
...
function renewSubscription ( $option, $id=null  ) {
	...
	selectSubscriptionPlanB($option, $id, $user->username, $user->name, 0 );
...
function saveSubscription ( $option ) {
	...
	$tables	= array();
	$tables	= $database->getTableList();
	if(in_array($mosConfig_dbprefix."cbe", $tables) || in_array($mosConfig_dbprefix."cbe_members", $tables)) {
		...
		case 'none':
			...
			$freePlanId	= 0;
		 	...
		 	if( ($entryPlanId == $planid) || ($freePlanId == $planid) )	{
				...
				// CB&CBE Integration
				if(in_array($mosConfig_dbprefix."cbe", $tables) || in_array($mosConfig_dbprefix."cbe_members", $tables)) {
					$messagesToUser = saveCBSubscription( $option, $row, $generatedPassword, $pwd );
				}
				userGatewayProcess( $userid, $objplan, 'None', $invoice );
				if($messagesToUser) {
					// We gonna use CB message instead of default thanks page
					echo $messagesToUser;
				} else {
					mosRedirect( $mosConfig_live_site . "/index.php?option=$option&task=thanks&free=1" );
				}
				return;
		 	} else {
				...
				$database->query();
				mosRedirect( $mosConfig_live_site . "/index.php?option=$option&task=cancel" );
		 		return;
		 	}
	}
 	// get the payment plan
	...
	// CB&CBE Integration
	if(in_array($mosConfig_dbprefix."cbe", $tables) || in_array($mosConfig_dbprefix."cbe_members", $tables)) {
		// $messageToUser will be lost here...
		// Because some gateways does not return any variable you pass
		$messagesToUser	= saveCBSubscription( $option, $row, $generatedPassword, $pwd );
	}
...
function activateFT ( $option ) {
	global $database, $mosConfig_offset_user, $mosConfig_live_site;
	...
	if ($planid > 0) {
		...
		switch(strtolower($method)) {
		 ...
		 mosRedirect( $mosConfig_live_site . "/index.php?option=$option&task=thanks&renew=$renew&free=$free" );
	}
}
...
function selectSubscriptionPlanB ( $option, $userid, $username, $name, $intro=0 ) {
	// No AJAX
	...
	if( $userid > 0 ) { // This is a renew subscription
		...
		unset($where);
		$groups	= GeneralInfoRequester::getLowerACLGroup( $mygroupid );
		...
		// Set the minimum gid to see the plan, all plans with lower gids
	 	$database->setQuery("SELECT a.mingid"
	 	. "\nFROM #__acctexp_plans AS a"
	 	. "\nINNER JOIN #__core_acl_aro_groups AS b ON a.mingid = b.group_id"
	 	. "\nWHERE a.active=1"
	 	. "\nORDER BY b.lft"
	 	. "\nLIMIT 0,1"
	 	);
		...
		. (count( $where ) ? "\nWHERE " . implode( ' AND ', $where ) : "")
	 	...
	 	if($hasEntryPlan > 0) {
			...
			if(in_array($mosConfig_dbprefix."cbe", $tables) || in_array($mosConfig_dbprefix."cbe_members", $tables)) {
				...
				Payment_HTML::subscribeCBForm( $option, $mosConfig_emailpass, $rowFields, $rowFieldValues, $var, $regErrorMSG, 2, $cfg->checkusername );
			} else {
			...
			. "\nORDER BY ordering"
		 	...
		 	$rows = $database->loadObjectList();
		 	if(count($rows) == 0) {
				mosRedirect( $mosConfig_live_site . "/index.php?mosmsg=" . _NOPLANS_ERROR );
		 		return;
		 	}
		 	if ($database->getErrorNum()) {
		 		echo $database->stderr();
...
function buildForm ( $option, $planid, $userid, $username, $name, $recurring, $numberOfSteps=3, $processor ) {
	...
	if($userid) {
		...
		if(in_array($mosConfig_dbprefix."cbe", $tables) || in_array($mosConfig_dbprefix."cbe_members", $tables)) {
			...
			$rowFieldValues	= null;
			getFieldsForCBForm( $rowFields, $rowFieldValues, $regErrorMSG );
			Payment_HTML::subscribeCBForm( $option, $mosConfig_emailpass, $rowFields, $rowFieldValues,$var, $regErrorMSG, $numberOfSteps, $configGeneral->checkusername );
		} else {
...
function processVKLIXNotification ( $option ) {
	...
	if ( ( $ssl_result == 0 ) && ( strcmp ( $ssl_result_message, "APPROVED") == 0 )) {
		...
		mosRedirect( $mosConfig_live_site . "/index.php?option=$option&task=thanks&renew=$renew" );
	} elseif ($ssl_result > 0) {
		// log for manual investigation
	}
}
...
function processepsNotification ( $option ) {
	...
	$HaendlerPIN		= $row_Config_epsnetpay->epsmerchantpin;
	$sapPopRequestor	= $row_Config_epsnetpay->epsmerchantid;
...
function thanks ( $option, $renew, $free ) {
	...
	$messagesToUser	= null;
	// CB&CBE Integration
	if( ((in_array($mosConfig_dbprefix."cbe", $tables)) || (in_array($mosConfig_dbprefix."cbe_members", $tables)) ) && ( $renew == 0) ) {
		if ($mosConfig_emailpass == "1" && $ueConfig['reg_admin_approval']=="1" && $ueConfig['reg_confirmation']=="0"){
			...
			if($free) {
				$messagesToUser	= _SUB_COMPLETE_FREE;
			} else {
				$messagesToUser	= _SUB_COMPLETE;
			}
		}
		$msg	= $messagesToUser;
	} elseif( $renew == 0 ) {
	...
	// TODO: Include Variable to db
	if ($cfg->customthankyoupage) {
		mosRedirect($cfg->customthankyoupage);
	} else {
...
function cancelPayment ( $option ) {
	...
	// TODO: Include Variable to db
	if ($cfg->customcancelpage) {
		mosRedirect($cfg->customcancelpage);
	} else {
...
function userPayPalProcess ( $item_number, $payment_date, &$plan, $mc_gross, $mc_currency, $recurring, $invoice_number ) {
	global $mainframe, $database, $mosConfig_useractivation, $mosConfig_offset_user, $mosConfig_dbprefix, $acl;
	...
	if($id) {
		...
		$newUser        = 1;
		// Delete any manually configured expiration date
		$database->setQuery( "DELETE FROM #__acctexp WHERE userid=$item_number" );
		$database->query();
	}
	...
	if(strcmp($db->status, 'Pending') == 0 || $newUser) {
		...
		if(!((in_array($mosConfig_dbprefix."cbe", $tables)) || (in_array($mosConfig_dbprefix."cbe_members", $tables))) ) {	// CB&CBE Integration
	...
	$gid_name = $acl->get_group_name($plan->gid, 'ARO');
	$query = "UPDATE #__users SET block='0', gid = '$plan->gid', usertype='$gid_name' WHERE id = '$item_number'";
	$database->setQuery( $query );
	$database->query() or die( $database->stderr() );

	$query = "UPDATE #__session SET usertype='$gid_name' WHERE userid = '$item_number'";
	$database->setQuery( $query );
	$database->query() or die( $database->stderr() );
	//End Moxie Mod
...
function userGatewayProcess ( $item_number, &$plan, $processor_name, $invoice_number ) {
	...
	global $mosConfig_mailfrom, $mosConfig_fromname, $mosConfig_sitename, $mosConfig_dbprefix, $acl;
	global $mosConfig_live_site;
	...
	//Moxie Mod - updated to add usertype to users table and update session table for immediate access to usertype features
	$gid_name = $acl->get_group_name($plan->gid, 'ARO');
	/** Code for enable user activation
	if ($mosConfig_useractivation=="1"){
		$query = "UPDATE #__users SET gid = '$plan->gid', usertype='$gid_name' WHERE id = '$item_number'";
		$database->setQuery( $query );
		$database->query() or die( $database->stderr() );

		$objUser = new mosUser( $database );
		$objUser->load($item_number);
		if($objUser->id) {
			$subject = sprintf (_SEND_SUB, $objUser->name, $mosConfig_sitename);
			$subject = html_entity_decode($subject, ENT_QUOTES);

			$message = sprintf (_USEND_MSG_ACTIVATE, $objUser->name, $mosConfig_sitename, $mosConfig_live_site."/index.php?option=com_registration&task=activate&activation=".$objUser->activation, $mosConfig_live_site, $objUser->username, $objUser->pwd);
			...
			mosMail($adminEmail2, $adminName2, $objUser->email, $subject, $message);
		}
	} else { */
		$query = "UPDATE #__users SET block='0', gid = '$plan->gid', usertype='$gid_name' WHERE id = '$item_number'";
		$database->setQuery( $query );
...
// PK START
function loginValidate_acct_cbe( $username=null ) {
	global $acl,$database,$_COOKIE,$_POST,$mainframe, $ueConfig,$mainframe,$_PLUGINS;
	global $mosConfig_offset_user, $mosConfig_dbprefix, $mosConfig_live_site;
	$resultError = null;
	...
	if ($usr->id) {
		...
		if ($expiration) {
			...
			if (strcmp($status, 'Pending') == 0) {
				...
				mosRedirect( $mosConfig_live_site . "/index.php?option=com_acctexp&task=pending&userid=" . $usr->id );
				exit();
			}
			if ( (strcmp($status, 'Closed') == 0) || (strcmp($status, 'Cancelled') == 0) || ( ($expstamp > 0) && ( $expstamp-(time()+$mosConfig_offset_user*3600) < 0 ) )) {
				...
				mosRedirect( $mosConfig_live_site . "/index.php?option=com_acctexp&task=expired&userid=" . $usr->id . "&expiration=" . strftime(_ACCT_DATE_FORMAT, $expstamp));
				exit();
			}
		}
	}
...
function acct_login ( $username=null,$passwd=null ) {
	/**
	* Fuction from Joomla Community Builder
	* @version $Id: cbe.php 326 2006-05-09 01:50:12Z beat $
	* @package Community Builder
