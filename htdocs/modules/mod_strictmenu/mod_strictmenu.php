<?php
/**
* @version $Id: mod_strictmenu.php 5941 2006-12-06 13:23:38Z predator $
* @package Joomla
...
if (!defined( '_MOS_strictmenu_MODULE' )) {
	/** ensure that functions are declared only once */
	define( '_MOS_strictmenu_MODULE', 1 );

	/**
	* Utility function for writing a menu link
	*/
	function mosGetstrictmenuLink( $mitem, $level=0, &$params, $open=null, $parents=null ) {
		...
		if(isset( $parents ) && in_array( $mitem->id, $parents ) ){
			$menuclass = 'mainlevel'. $params->get( 'class_sfx' ) . '_parent';
		} else {
		...
		switch ($mitem->browserNav) {
			...
			case 2:
				...
				$txt = "<a href=\"#\" onclick=\"javascript: window.open('". $mitem->link ."', '', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=1024,height=550'); return false\" class=\"$menuclass\" ". $id .">". $mitem->name ."</a>\n";
				break;
	...
	*/
	function mosShowStrictVIMenu(  &$params ) {
		...
		mosRecurseStrictVIMenu( 0, 0, $children, $open, $indents, $params, $parents );
	...
	* hierarchial menu
	*/
	function mosRecurseStrictVIMenu( $id, $level, &$children, &$open, &$indents, &$params, $parents=null ) {
		if (@$children[$id]) {
			...
			foreach ($children[$id] as $row) {
				...
				echo mosGetstrictmenuLink( $row, $level, $params, $open, $parents );

				// show menu with menu expanded - submenus visible
				if ( !$params->get( 'expand_menu' ) ) {
					if ( in_array( $row->id, $open )) {
						mosRecurseStrictVIMenu( $row->id, $level+1, $children, $open, $indents, $params );
					}
				} else {
					mosRecurseStrictVIMenu( $row->id, $level+1, $children, $open, $indents, $params );
				}
	...
	*/
	function mosShowStrictHFMenu(  &$params, $style=0 ) {
		global $database, $my, $cur_template, $Itemid;
		...
		$links = array();
		foreach ($rows as $row) {
			$links[] = mosGetstrictmenuLink( $row, 0, $params );
		}
...
switch ( $params->get( 'menu_style', 'vert_indent' ) ) {
	case 'list_flat':
		mosShowStrictHFMenu( $params, 1 );
		break;
	
	case 'horiz_flat':
		mosShowStrictHFMenu( $params, 0 );
		break;
	
	default:
		mosShowStrictVIMenu( $params );
		break;
}
?>